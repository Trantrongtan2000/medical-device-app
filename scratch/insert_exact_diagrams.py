...
diagrams_dict = {
    'data-group="CS.TTBYT.04"': ("SƠ ĐỒ QUY TRÌNH KIỂM ĐỊNH & HIỆU CHUẨN THIẾT BỊ Y TẾ (CS.TTBYT.04)", """graph LR
    ...
    'data-group="TA5.TTBYT.QT.01"': ("SƠ ĐỒ KIỂM SOÁT CHẤT LƯỢNG NƯỚC R.O THẬN NHÂN TẠO (QT.01)", """graph TD
    ...
    'data-group="TA5.TTBYT.QT.02"': ("SƠ ĐỒ VẬN HÀNH HỆ THỐNG R.O THẬN NHÂN TẠO (QT.02)", """graph LR
    ...
    'data-group="TA5.TTBYT.QT.03"': ("SƠ ĐỒ VẬN HÀNH & KIỂM TRA HỆ THỐNG KHÍ Y TẾ TRUNG TÂM (QT.03)", """graph TD
    ...
    'data-group="TA5.TTBYT.QT.04"': ("SƠ ĐỒ BÀN GIAO, LẮP ĐẶT & NGHIỆM THU ĐƯA VÀO SỬ DỤNG (QT.04)", """graph LR
    ...
    'data-group="TA5.TTBYT.QT.05"': ("SƠ ĐỒ VẬN HÀNH & BẢO QUẢN THIẾT BỊ TẠI KHOA PHÒNG (QT.05)", """graph TD
    ...
    'data-group="TA5.TTBYT.QT.06"': ("SƠ ĐỒ BẢO TRÌ ĐỊNH KỲ (PM) & SỬA CHỮA BÁO HỎNG SPEEDMAINT (QT.06)", """graph LR
    ...
    'data-group="TA5.TTBYT.QT.07"': ("SƠ ĐỒ QUY TRÌNH THANH LÝ THIẾT BỊ Y TẾ (QT.07)", """graph LR
    ...
    'data-group="TA5.TTBYT.QT.08"': ("SƠ ĐỒ ĐIỀU CHUYỂN THIẾT BỊ GIỮA CÁC KHOA PHÒNG (QT.08)", """graph LR
    ...
    'data-group="TA5.TTBYT.QT.09"': ("SƠ ĐỒ GIAO NHẬN BÌNH KHÍ Y TẾ DI ĐỘNG (QT.09)", """graph LR
...
injected = 0
for needle, (title, mm_code) in diagrams_dict.items():
    wrapper_html = f"""
...
"""
    # Find needle and insert wrapper_html after the closing </div> of .block-head
    pos = content.find(needle)
    if pos != -1:
        end_head_idx = content.find('</div>', pos) # closes <div class="block-code">
        if end_head_idx != -1:
            end_block_head_idx = content.find('</div>', end_head_idx + 6) # closes <div class="block-head">
            if end_block_head_idx != -1:
                insert_pos = end_block_head_idx + 6
                content = content[:insert_pos] + "\n" + wrapper_html + content[insert_pos:]
                injected += 1
                print(f"✅ Injected into {needle}")
